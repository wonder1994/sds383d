...
mean_matrix = np.flip(np.reshape(mean_star, (numpoints, numpoints)), axis = 0)
prob_matrix = 1./(1 + np.exp(-mean_matrix))
sd_matrix = np.flip(np.reshape(sd_star, (numpoints, numpoints)), axis = 0)

plt.pcolormesh(prob_matrix, )
plt.colorbar()
curves = 10
